...
i32 std_ll_t_insert_at_beginning(std_ll_t * ll, std_ll_t_node_t * node) {
    if (ll -> m_head == NULL) {
        ll -> m_head = node;
...
i32 std_ll_t_insert_at_end(std_ll_t * ll, std_ll_t_node_t * node) {
    if (ll -> m_head == NULL) {
...
i32 std_ll_t_remove(std_ll_t * ll, std_ll_t_node_t * remove_node) {
    ...
    while (index < ll -> m_size && node != NULL) {
        if (remove_node -> m_data == node -> m_data && remove_node -> m_type == node -> m_type) {
            ...
            for (i32 i = index; i < ll -> m_size - 1; i++) {
